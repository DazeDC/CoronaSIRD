...
/* percentage of remaining susceptibles to reach before starting the vaccination of the next age group */
param vaccination_threshold = 0.1;
...
label alive = {S[i for i in [0, N]], I[i for i in [0, N]], R[i for i in [0, N]], V}
...
/* Susceptibles to vaccinated rules */
...
rule s_to_v_O50U60 {
    [#S[0] <= (vaccination_threshold * n_S0_init)] S[1] -[ 1.0 ]-> V
}

rule s_to_v_O60U70 {
    [#S[1] <= (vaccination_threshold * n_S1_init)] S[2] -[ 1.0 ]-> V
}

rule s_to_v_O70U80 {
    [#S[2] <= (vaccination_threshold * n_S2_init)] S[3] -[ 1.0 ]-> V
}

rule s_to_v_O80U90 {
    [#S[3] <= (vaccination_threshold * n_S3_init)] S[4] -[ 1.0 ]-> V
}

rule s_to_v_O90 {
    [#S[4] <= (vaccination_threshold * n_S4_init)] S[5] -[ 1.0 ]-> V
}

measure n_infected = #infected;

measure n_dead = #D[0] + #D[1] + #D[2] + #D[3] + #D[4] + #D[5];
